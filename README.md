...
수업도우미를 켠 뒤, 보드의 브라우저에서 이 주소를 열면 됩니다.

```
…/board-check/          예: http://127.0.0.1:43110/board-check/
```

정확한 주소는 실행 파일 옆의 `교가-확인.txt` 에 «화면 주소» 로 적혀 있습니다.
그 뒤에 `board-check/` 만 붙이면 됩니다.

컴퓨터로는 흉내 낼 수 없는 것(진짜 터치·소리·마이크·글꼴·보드 성능)을
10분 만에 확인하는 화면입니다. 끝나면 결과가 글로 정리됩니다.
