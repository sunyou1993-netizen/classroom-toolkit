...
const 표시들 = {
  ...
  'nav-extra-cards': '첫 화면 목록에 퀴즈 카드 (원본에는 퀴즈로 가는 길이 없습니다)',
};
...
const 코드속고침 = {
  ...
  /* 사다리 두 가지는 «글자 그대로» 가 아니라 «모양» 으로 찾습니다.
   * AI Studio 원본에도 같은 고침을 넣어 두었으므로, 앞으로는 새로 빌드한
   * 번들에 처음부터 들어 있습니다. 그때 압축기가 변수 이름을 다르게 지으면
   * (l → d 처럼) 글자 대조는 «빠졌다» 고 잘못 말합니다. */
  '사다리-층수': { 설명: '사다리 층 수를 인원에 맞춤 (공정하게)',
    찾을것: /const [A-Za-z_$]+=\[\],[A-Za-z_$]+=Math\.max\(6,\([A-Za-z_$]+-1\)\*3\);/ },
  '사다리-높이': { 설명: '사다리 가로줄 높이를 고르게',
    찾을것: /level:\.12\+\([A-Za-z_$]+\.lvl\+1\)\*\(\.76\/\([A-Za-z_$]+\+1\)\)/ },
};

/* 찾을것이 글자면 포함 여부로, 모양(정규식)이면 맞는지로 봅니다 */
const 들었나 = (s, 찾을것) =>
  찾을것 instanceof RegExp ? 찾을것.test(s) : s.includes(찾을것);
...
function 지금상태() {
  ...
  for (const f of 코드파일들()) {
    let s;
    try { s = fs.readFileSync(path.join(ROOT, f), 'utf8'); } catch { continue; }
    for (const [이름, 것] of Object.entries(코드속고침)) if (들었나(s, 것.찾을것)) 상태[이름].push(f);
  }
  return 상태;
}
